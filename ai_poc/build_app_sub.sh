...
for subdir in $(ls -d */); do
      ...
      if [ "$subdir_name" = "llamac" ] && { [ "$curr_pro" = "llamac" ] || [ "$curr_pro" = "all" ]; }; then
            ...
            make -j && make install
            # Keypoint FIFO reader: build with Linux (glibc) toolchain, not musl
            if [ "$subdir_name" = "dual_cam_pose_offline" ]; then
                  cd ..
                  mkdir -p out_reader_glibc
                  cd out_reader_glibc
                  if [ -d "/home/timot/linux_toolchain/Xuantie-900-gcc-linux-6.6.0-glibc-x86_64-V3.0.2/bin" ]; then
                        export PATH="$PATH:/home/timot/linux_toolchain/Xuantie-900-gcc-linux-6.6.0-glibc-x86_64-V3.0.2/bin"
                  else
                  ...
                  if [ "$is_01studio" -eq 1 ]; then
                        cmake -DCMAKE_BUILD_TYPE=Release \
                              -DCMAKE_INSTALL_PREFIX="$(pwd)" \
                              -DCMAKE_TOOLCHAIN_FILE=../cmake/Riscv64_glibc.cmake \
                              -Ddual_cam_pose_offline=ON \
                              -DBUILD_KEYPOINT_READER_GLIBC=ON \
                              -DDISPLAY_MODE=${display_mode} \
                              ..
                  else
                        cmake -DCMAKE_BUILD_TYPE=Release \
                  ...
                  make -j keypoint_fifo_reader.elf && make install
                  cp -f bin/keypoint_fifo_reader.elf ../out/bin/
                  cd ../out
            fi
      ...
      if [ "$subdir_name" = "pose_detect_sensor38" ]; then
            cp out/bin/pose_detect_sensor38.elf ${k230_bin}/$subdir_name
            cp -a ../../big/kmodel/ai_poc/kmodel/yolov8n-pose.kmodel ${k230_bin}/$subdir_name
            cp -a ../../big/kmodel/ai_poc/images/bus.jpg ${k230_bin}/$subdir_name
            cp -a shell/pose_detect_sensor38_*.sh ${k230_bin}/$subdir_name
      fi

      if [ "$subdir_name" = "pose_detect_sensor38_json" ]; then
            cp out/bin/pose_detect_sensor38_json.elf ${k230_bin}/$subdir_name
            cp -a ../../big/kmodel/ai_poc/kmodel/yolov8n-pose.kmodel ${k230_bin}/$subdir_name
            cp -a ../../big/kmodel/ai_poc/images/bus.jpg ${k230_bin}/$subdir_name
            cp -a shell/pose_detect_sensor38_json_*.sh ${k230_bin}/$subdir_name
      fi

      if [ "$subdir_name" = "dual_cam_pose_offline" ]; then
            cp out/bin/dual_cam_pose_offline.elf ${k230_bin}/$subdir_name
            cp out/bin/keypoint_fifo_reader.elf ${k230_bin}/$subdir_name
            cp -a ../../big/kmodel/ai_poc/kmodel/yolov8n-pose.kmodel ${k230_bin}/$subdir_name
            cp -a ../../big/kmodel/ai_poc/images/bus.jpg ${k230_bin}/$subdir_name
            cp -a shell/dual_cam_pose_offline_keypoint_*.sh ${k230_bin}/$subdir_name
      fi
      ...
      if [ "$subdir_name" = "pose_det_rtsp_plug" ]; then
            ...
            cp pose_det_rtsp_plug/little/out/poseKeypointReader ${k230_bin}/$subdir_name
